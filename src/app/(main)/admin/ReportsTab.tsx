...
export default function ReportsTab() {
  ...
  const exportReport = () => {
    ...
    filteredOrders.forEach(order => {
      ...
      (order.items || []).forEach((item: any) => {
        ...
        csvContent += `"${date}","${order.orderNumber}","${customerName}","PESANAN PRODUK","${productName}","${variantName}","${quantity}","${formatCurrency(price)}","${formatCurrency(total)}"\n`;
      });
    });
    ...
    filteredServiceOrders.forEach(serviceOrder => {
      ...
      csvContent += `"${date}","${serviceOrder.orderNumber || '-'}","${customerName}","JASA SERVICE","${serviceItem}","-","1","-","${formatCurrency(serviceTotal)}"\n`;
      ...
      if (serviceOrder.spareparts && serviceOrder.spareparts.length > 0) {
        serviceOrder.spareparts.forEach((sp: any) => {
          ...
          csvContent += `"${date}","${serviceOrder.orderNumber || '-'}","${customerName}","SPAREPART SERVICE","${productName}","${variantName}","${quantity}","${formatCurrency(price)}","${formatCurrency(total)}"\n`;
        });
      }
    });
    ...
    csvContent += `"Total Revenue Produk","","","","","","","","${formatCurrency(productRevenue)}"\n`;
    csvContent += `"Total Revenue Service","","","","","","","","${formatCurrency(serviceRevenue)}"\n`;
    csvContent += `"TOTAL REVENUE","","","","","","","","${formatCurrency(productRevenue + serviceRevenue)}"\n`;
  ...
  const getReportTypeLabel = () => {
    switch (reportType) {
      ...
      case 'yearly': return 'LAPORAN TAHUNAN';
      ...
      <div className="space-y-6">
        ...
        <div className="card overflow-hidden">
          <div className="p-4 border-b border-[var(--border-color)] bg-[var(--bg-elevated)] flex items-center justify-between">
            <h4 className="font-semibold">Laporan Laba Rugi Per Produk</h4>
            <button
        ...
        {loading ? (
          ...
          <div>
            <h4 className="font-semibold mb-4">
              ...
              {reportType === 'yearly' && 'Laporan Tahunan'}
              {reportType === 'fast-moving' && 'Produk Laku Keras'}
              {reportType === 'profit-loss' && 'Laporan Laba Rugi'}
            </h4>
            {generateReport()}
          </div>
        )}
        ...
        <div className="hidden">
          <div ref={printRef}>
            ...
            <div className="footer">
              <p>Laporan ini dibuat secara otomatis oleh sistem Satria Elektronik</p>
